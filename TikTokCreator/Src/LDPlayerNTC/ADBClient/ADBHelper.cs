...
namespace LDPlayerAndADBController.ADBClient
{
    public class ADBHelper
    {
        ...
        /// <returns></returns>
        public static string ExecuteADB_Result(string command, int timeoutInSeconds = 10)
        {
            string result = "";
            try
            {
                Process process = new Process();
                process.StartInfo.FileName = $"{PathFolderADB}\\adb.exe";
        ...
        ///
        ////
        public static void DeletePNGFilesAndScanMedia(string deviceId, string directoryPath)
        {
            // Xóa tất cả các tệp tin PNG trong thư mục
            string deleteCommand = $"shell find {directoryPath} -name \"*.png\" -type f -delete";
            var file = ADB(deviceId, deleteCommand);

            if (!string.IsNullOrEmpty(file))
            {
                string scanCommand = $"shell am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d file://{file}";
                ADB(deviceId, scanCommand);
            }
        ...
        public static void Delay(int timeDelay)
        {
            try
            {
                var time = timeDelay * 1000;
                Thread.Sleep(time);
            }
            catch (Exception ex)
            {
                Log.Error($"{nameof(ADBHelper)}, params; {nameof(Delay)},timeDelay; {timeDelay}, Error; {ex.Message}, Exception; {ex}");
                return;
            }
        }
        public static void Delay(int timeDelayStart = 2, int timeDelayStop = 5)
        {
            try
            {
                Random random = new Random();
                var time = (random.Next(timeDelayStart, timeDelayStop) * 1000);
                Thread.Sleep(time);
            }
            catch (Exception ex)
            {
                Log.Error($"{nameof(ADBHelper)}, params; {nameof(Delay)}, Error; {ex.Message}, Exception; {ex}");
                return;
            }
        }
    }
